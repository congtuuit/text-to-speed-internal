      ...
      <section className="composer-grid">
        ...
        <div className="glass-panel side-card">
          ...
          <div style={{ marginBottom: "0.75rem", fontSize: "0.9rem", color: "var(--text-muted)" }}>
            <span>{(() => { const sv = savedVoices.find(s => s.voice_type === voice && (s.seed || '') === (createVoiceSeed || '')) || savedVoices.find(s => s.voice_type === voice); return <>{t("create.voice")}: <strong>{sv ? sv.name : voice}</strong>{sv ? <span style={{ marginLeft: "0.5rem" }}>({t("voices.seed")}: {sv.seed || "-"})</span> : null}</>; })()}</span>
          </div>
