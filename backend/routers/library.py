...
def delete_saved_voice(voice_id: int, request: Request, db: Session = Depends(get_db)):
    user = _current_user_from_request(request, db)
    if not user:
        raise HTTPException(status_code=401, detail="Chưa đăng nhập")
    voice = db.query(models.SavedVoice).filter(models.SavedVoice.id == voice_id, models.SavedVoice.owner_id == user.id).first()
    if not voice:
